...
{  
    ...
    {  
        ...
        if (!isset($this->_REDIS)) {
            ...
            $this->connect($this->_HOST, $this->_PORT, $this->_TIMEOUT, $this->_CTYPE,$this->_AUTH);  
        }  
     ...
     * 连接redis服务器 
     */  
    private function connect($host,$port,$timeout,$type,$auth)  
    {  
        switch ($type) {  
            case 1:  
                $this->_REDIS->connect($host, $port, $timeout,$type);  
				$this->_REDIS->auth($auth);
            break;  
            case 2:  
                $this->_REDIS->pconnect($host, $port, $timeout,$type); 
				$this->_REDIS->auth($auth);
            break;  
            default:  
            break;  
